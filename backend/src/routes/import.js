    ...
    try {
      ...
      if (guardianName) {
        if (guardianCache[guardianName]) {
          ...
          if (ex.rows[0]) { guardianId=ex.rows[0].id; }
          else {
            const g=await client.query('INSERT INTO dance_camp_guardians (name,relationship,address,city_state_zip,dob,email,cell_phone,work_phone) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id',
              [guardianName,nullify(row['Parent Relationship']),nullify(row['Parent Address']),nullify(row['Parent City/State/Zip']),normalizeDate(row['Parent DOB']),nullify(row['Parent Email']),nullify(row['Parent Cell Phone']),nullify(row['Parent Work Phone'])]);
      ...
      if (ecName) {
        const ecEx=await client.query('SELECT id FROM danceakerbase
￼
Type / to search
￼
￼
...
Code
_camp_emergency_contacts WHERE camper_id=$1 AND name=$2',[camperId,ecName]);
...
async function importMembers(rows,userId,client){
  let imported=0,skipped=0;const errors=[];
  for(const[i,row]of rows.entries()){const rn=i+2;try{const name=nullify(row['Name']??row['Full Name']);if(!name){skipped++;continue;}await client.query('INSERT INTO members (name,email,major,status,joined) VALUES ($1,$2,$3,$4,$5) ON CONFLICT DO NOTHING',[name,nullify(row['Email']),nullify(row['Major']),nullify(row['Status'])??'active',normalizeDate(row['Joined'])??new Date().toISOString().slice(0,10)]);imported++;}catch(err){errors.push({row:rn,message:err.message});}}
  return{imported,skipped,errors};
}

async function importEquipment(rows,userId,client){
  let imported=0,skipped=0;const errors=[];
  for(const[i,row]of rows.entries()){const rn=i+2;try{const name=nullify(row['Name']);if(!name){skipped++;continue;}await client.query('INSERT INTO equipment (name,category,status,location,serial,note) VALUES ($1,$2,$3,$4,$5,$6) ON CONFLICT DO NOTHING',[name,nullify(row['Category'])??'Other',nullify(row['Status'])??'available',nullify(row['Location']),nullify(row['Serial']??row['Serial Number']),nullify(row['Note']??row['Notes'])]);imported++;}catch(err){errors.push({row:rn,message:err.message});}}
  return{imported,skipped,errors};
}
...
async function importPartners(rows,userId,client){
  ...
  for(const[i,row]of rows.entries()){const rn=i+2;try{const name=nullify(row['Name']??row['Organization']);if(!name){skipped++;continue;}const mouRaw=nullify(row['MOU Status']??row['MOU'])?.toLowerCase().replace(/s/g,'_');const mou=validMou.includes(mouRaw)?mouRaw:'none';await client.query('INSERT INTO partners (name,kind,contact,role,email,phone,mou_status,notes) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT DO NOTHING',[name,nullify(row['Kind']??row['Type'])??'Nonprofit',nullify(row['Contact']),nullify(row['Role']),nullify(row['Email']),nullify(row['Phone']),mou,nullify(row['Notes'])]);imported++;}catch(err){errors.push({row:rn,message:err.message});}}
  return{imported,skipped,errors};
}

async function importCerts(rows,userId,client){
  let imported=0,skipped=0;const errors=[];
  for(const[i,row]of rows.entries()){const rn=i+2;try{const name=nullify(row['Name']??row['Certification']);if(!name){skipped++;continue;}const vd=parseInt(row['Valid Days']??row['valid_days']);await client.query('INSERT INTO certifications (name,valid_days) VALUES ($1,$2) ON CONFLICT DO NOTHING',[name,isNaN(vd)?null:vd]);imported++;}catch(err){errors.push({row:rn,message:err.message});}}
  return{imported,skipped,errors};
}

const importers={dance_camp:importDanceCamp,members:importMembers,equipment:importEquipment,partners:importPartners,certifications:importCerts};
...
router.post('/', requireAuth, upload.single('file'), async (req,res) => {
  ...
  } catch(err){await client.query('ROLLBACK');return res.status(500).json({error:'Import failed: '+err.message});}
  finally{client.release();}
  await pool.query('INSERT INTO import_logs (filename,sheet_type,rows_found,rows_imported,rows_skipped,errors,imported_by) VALUES ($1,$2,$3,$4,$5,$6,$7)',
    [req.file.originalname,type,rows.length,result.imported,result.skipped,result.errors.length?JSON.stringify(result.errors):null,req.user.id]).catch(()=>{});
